...
class CNNBinaryClassifierAug(nn.Module):
  def __init__(self):
    super().__init__()
    self.conv1 = nn.Conv2d(3, 32, 3)
    self.bn1 = nn.BatchNorm2d(32)
...
def train_aug(learning_rate, num_epochs, batch_size,
  ...
  # Сохраняем веса модели
  torch.save(model.state_dict(), 'model/model_augmentation_weights.pth')

  to_json("metrics/model_augmentation/train", "train_accuracies", train_accuracies)
  to_json("metrics/model_augmentation/train", "train_losses", train_losses)
  to_json("metrics/model_augmentation/validation", "val_accuracies", val_accuracies)
  to_json("metrics/model_augmentation/validation", "val_losses", val_losses)
